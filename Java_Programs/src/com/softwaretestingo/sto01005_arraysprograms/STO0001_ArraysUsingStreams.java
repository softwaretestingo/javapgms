...
public class STO0001_ArraysUsingStreams 
{
	/**
	 ...
	 * 
	 * Problem Statement: Stream Example
	 * 
	 */
	public static void main(String[] args) 
	{
		 ...
		 * vehicles.stream().map(s -> s.toLowerCase()).forEach(s ->
		...
		// numbers.stream().filter(s->s%2==0).forEach(s->System.out.println("Even
		// numbers in array is "+s));
		// numbers.stream().filter(s->s%2!=0).forEach(s->System.out.println("Odd numbers
		// in array is "+s));

		// Filter number starting with 2 from array
		// List<Integer>
		// numbersstartingwithTwo=numbers.stream().map(s->String.valueOf(s)).filter(s->s.startsWith("2")).map(Integer::valueOf).collect(Collectors.toList());
		...
		List<Integer> duplicatesInArray = numbers.stream().filter(s -> Collections.frequency(numbers, s) > 1)
		...
		/*
		 * List<Integer>
		 * ascending=numbers.stream().distinct().sorted().collect(Collectors.toList());
		 * System.out.println(ascending);
		 * 
		 * List<Integer>
		 * descending=numbers.stream().distinct().sorted(Collections.reverseOrder()).
		 * collect(Collectors.toList()); System.out.println(descending);
		 */

		// Get first 5 values from array and sum of first 5 values
		/*
		 * List<Integer>
		 * firstFiveValues=numbers.stream().limit(5).collect(Collectors.toList());
		...
		/*
		 * List<Integer>
		 * skippedfirstFiveValues=numbers.stream().skip(5).collect(Collectors.toList());
